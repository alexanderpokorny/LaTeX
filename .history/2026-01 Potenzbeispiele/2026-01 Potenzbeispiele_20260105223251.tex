...
\section{Großes Potenzen-Training (Intensiv)}

\vspace{1cm}

\subsection{Warm-up: Die "gefährlichen" Basics}
...
\begin{itemize}
    ...
    \item[d)] $-7^0 = $ \underline{\hspace{2cm}} \textit{(Vorsicht!)}
...
\begin{itemize}
    ...
    \item[e)] $10^7 : 10^{-2} = $ \quad \textit{(Tipp: Minus minus ergibt plus)}
\end{itemize}
...
\begin{itemize}
    \item[a)] $(2x)^3 =$ \quad \textit{(Rechne $2^3$ und $x^3$)}
    \item[b)] $(3a^2)^2 =$
    \item[c)] $(5x^3 y^2)^2 =$
    \item[d)] $(-2a^3)^3 =$ \quad \textit{(Vorzeichen beachten!)}
    \item[e)] $(-3x^2)^2 =$
    \item[f)] $\left(\dfrac{x}{2}\right)^3 =$
    \item[g)] $\left(\dfrac{2a^2}{3b^3}\right)^2 =$
\end{itemize}

\subsection{Level 3: Komplexe Vereinfachungen (Der "Endgegner")}
...
\begin{itemize}
    \item[a)] $x^{-3} =$
    \item[b)] $2a^{-2} =$ \quad \textit{(Gehört der 2er in den Nenner? Nein!)}
    \item[c)] $(2a)^{-2} =$
    \item[d)] $\dfrac{1}{x^{-4}} =$
...
Verbinde die Terme, die das Gleiche bedeuten.
